...
public class ComputerPlayer extends Player{
    ...
    public void movePlayer(float deltaxComputer)
    {
        if (x > 0)
            deltaxComputer++;
        if(x + bitmap.getWidth() > canvasWidth)
            deltaxComputer--;
    }
    ...
    public void draw(Canvas c) {
        super.draw(c);
        c.drawCircle(startingPositionX + bitmap.getWidth() / 2, (canvasHeight - 8 * canvasHeight / 9) + bitmap.getHeight() / 2, bitmap.getWidth() * 1.25f, hilaPaint);
    }
}
